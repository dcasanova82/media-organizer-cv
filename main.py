...
def listfiles(dir_path):
    with os.scandir(dir_path) as it:
        for entry in it:
            if not entry.name.startswith('.') and entry.is_file() and not (
                    entry.name.endswith('.db') or entry.name == 'Folder.jpg') and (
                    entry.name.endswith('.JPG') or entry.name.endswith('.JPEG') or entry.name.endswith(
                'jpg') or entry.name.endswith('jpeg')):
